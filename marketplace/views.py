...
def markeplate(request: HttpRequest):
...
def vendor_details(request: HttpRequest, vendor_slug=None):
...
def add_to_cart(request: HttpRequest, food_id):
    if request.user.is_authenticated:
        if is_ajax(request):
            ...
            try:
                ...
                try:
                    checkCart: Cart = Cart.objects.get(user=request.user, 
                                        fooditem=fooditem)
...
def decrease_cart(request: HttpRequest, food_id):
    if request.user.is_authenticated:
        if is_ajax(request):
            # check if the food item exists
            try:
...
def delete_cart(request: HttpRequest, cart_id):
    if request.user.is_authenticated:
        if is_ajax(request):
            try:
